...
Execution controls can change without replacing training state: CUDA uses the compiled decoder by default; `--decoder-mode current` selects eager. The iteration/overnight runner's `--concurrent-games` overrides collection slots without altering search targets or the stored config identity, and records the override in run metrics/supervisor identity. Existing evaluation concurrency remains controlled by its config. It does not change training token batch size, LR, reuse or collection thresholds.
