...
Matched 100-game halving/SF2400 screens (budget 2048, roots 16, refutations 4, own expansions 3, depth 8) completed without unfinished games: ckpt34 **65%** (49W/32D/19L), actor15 **58%** (41W/34D/25L). Independent game bootstrap, stratified by model color, gave a difference of -7 points, CI -17.5 to +4. This does not establish a Stockfish regression at this sample size. It is distinct from the paired-prefix Gumbel test.

Outcome split clarifies the monitoring tradeoff: on decisive trajectories, accuracy fell from 81.9% to 72.3% and WDL CE rose .376 → .617. Draw-trajectory CE fell 20.52 → 4.21. Actor15's mean draw probability was 13.7% on decisive positions versus 16.4% on drawn positions: matching the overall draw rate is not sufficient position-level calibration.

The controlled final-phase fork changed only value weight 1.0 → 0.1, retaining identical weights, optimizer history, replay samples and 20 updates. Counters and sampler state matched. Monitoring policy CE changed 1.6487 → 1.6445, WDL accuracy 62.42% → 62.61%, and Brier .5160 → .5162: small changes, not a demonstrated repair. Its 100-game Gumbel screen completed at **31.5%, paired 95% CI 23.5–39.5%** against ckpt34 (25W/13D/62L). The difference versus the original actor15 was -6 points, paired CI -18.5 to +6: this does not establish that weight .1 is worse than weight 1, but it does not repair the regression. This fork retains 305 steps of historical optimizer/model adaptation; it does not test weight .1 from the beginning. The prepared two-hour trial was **not launched** because the repair screen failed. Production learning remains halted. The next experiment should compare fresh starts from ckpt34 on identical replay/exposure budgets, rather than extrapolating from this late-phase intervention.

The diagnosis completed **500 evaluation games**: two 100-game fresh Gumbel screens, two 100-game halving/SF2400 screens and the 100-game value-weight-fork screen. Every game completed. TensorBoard includes a separate `diagnosis-2026-09-13` run under the pilot's existing log directory; reports and counterfactual artifacts remain separate from production replay. No new best checkpoint or successful learning recipe is claimed.
